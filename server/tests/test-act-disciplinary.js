/**
 * TEST: ACT Access Canberra — Register of Disciplinary Actions
 *
 * PURPOSE
 *   Verifies that searchACTDisciplinary() returns a correctly shaped result when
 *   given an entity name that exists in the ACT Socrata disciplinary-actions
 *   register. The test self-discovers a fixture by querying the API for the first
 *   Builder record, then calls the full scraper function and confirms the same
 *   entity comes back.
 *
 ...
 *       → Socrata endpoint changed or no current Builder disciplinary records
 *     • Scraper throws
 ...
 *
 * USAGE
 *   node server/tests/test-act-disciplinary.js
 *   node server/tests/test-act-disciplinary.js --name "John Smith Constructions"
 *
 * EXIT CODE
 ...
 *       Check: GET https://data.act.gov.au/resource/avib-prrz.json?$where=occupation='Builder'&$limit=5
 *   "Step 2 FAIL: could not extract fixture name from raw records"
 *     → The 'licensee_name' field is missing or empty. Check the raw API response
 *       shape — fields may have been renamed in the Socrata dataset.
 *   "Step 3 FAIL: scraper threw"
 *     → searchACTDisciplinary() itself threw an exception. Check axios errors
 ...
 *       statement in searchACTDisciplinary() in server/scrapers/actLicences.js.
 *   "Step 4 WARN: 0 results returned"
 *     → The fixture name passed nameMatchesEntity filtering in Step 1 but was
 *       filtered out inside searchACTDisciplinary. Check:
 *       (a) BUILDING_OCCUPATIONS set — occupation of the raw record may differ
 *       (b) nameMatchesEntity — significant words may not appear in licensee_name
...
const { searchACTDisciplinary } = require(path.join(__dirname, '../scrapers/actLicences'));
const { pass, fail, step, warn, dump, header, summary } = require('./lib/helpers');

const RESOURCE_URL = 'https://data.act.gov.au/resource/avib-prrz.json';
const PORTAL_URL = 'https://www.data.act.gov.au/Business-and-Industry/Register-Of-Disciplinary-Actions/avib-prrz';
...
(async () => {
  header('ACT Access Canberra — Register of Disciplinary Actions Test');
  let passed = 0;
  let failed = 0;

  // ── Step 1: Discover fixture via raw Socrata API ───────────────────────────
  step('Step 1: Querying ACT Socrata API for Builder disciplinary records...');
  ...
  if (!suppliedName) {
    ...
    pass('Step 1', `Found ${rawRecords.length} Builder disciplinary record(s) from Socrata API`);
    passed++;
  } else {
  ...
  if (!fixtureName) {
    // Prefer a record with a non-empty licensee_name that produces meaningful sig words
    for (const rec of rawRecords) {
      const candidate = (rec.licensee_name || '').trim();
    ...
    // Fall back to the first licensee_name even if no sig words
    if (!fixtureName && rawRecords.length > 0) {
      fixtureName = (rawRecords[0].licensee_name || '').trim();
    }
  }
  ...
  if (!fixtureName) {
    fail('Step 2',
      ...
      'The "licensee_name" field may be empty or the dataset shape has changed.\n' +
      'Raw records:',
  ...
  if (rawRecords.length > 0) {
    ...
    step(`  Sample raw record: occupation=${s.occupation}, licensee_name=${s.licensee_name}, ` +
         `action_type=${s.action_type || ''}, action_date=${s.action_date || ''}`);
  }
  passed++;

  // ── Step 3: Call scraper + validate return shape ───────────────────────────
  step(`Step 3: Calling searchACTDisciplinary("${fixtureName}", "", [])...`);

  let result;
  try {
    result = await searchACTDisciplinary(fixtureName, '', []);
  } catch (e) {
    fail('Step 3', `searchACTDisciplinary threw: ${e.message}`, e.stack);
    failed++;
    summary(passed, failed);
    process.exit(1);
  }
  ...
  if (result.category !== 'regulatory') {
    fail('Step 3', `Expected category "regulatory", got "${result.category}"`);
    failed++;
    summary(passed, failed);
    process.exit(1);
  }
  ...
  if (result.results.length === 0) {
    warn(
      `0 results returned for "${fixtureName}". The ACT disciplinary register may have no ` +
      'active Builder records matching this name after nameMatchesEntity filtering.\n' +
      'Checks to perform:\n' +
      '  (a) Confirm occupation is "Builder" / "Building Surveyor" / "Building Assessor" in the raw record\n' +
      '  (b) Confirm significant words from the fixture name appear in licensee_name\n' +
  ...
  if (firstWord) {
    ...
    if (!found) {
      warn(
        ...
        'This may be expected if the register displays the name in a different form.'
      );
    } else {
