/**
 * TEST: ACT Access Canberra — Builder & Contractor Licence Register
 *
 * PURPOSE
 *   Probes the ACT Access Canberra licence check to confirm:
 ...
 *   URL from links.js: https://www.accesscanberra.act.gov.au/licence-and-registration/check-a-licence
 *
 * REQUIREMENTS
 *   No API keys. Tries axios; falls back to Puppeteer if JS-rendered.
 *
 * USAGE
 *   node server/tests/test-act-licence.js
 *   node server/tests/test-act-licence.js --name "Smith Building"
 *
 * EXIT CODE
...
const BASE = 'https://www.accesscanberra.act.gov.au';
const LICENCE_URL = `${BASE}/licence-and-registration/check-a-licence`;
// ACT may use a ServiceNow or JADU portal — try alternative paths
const ALT_URLS = [
  `${BASE}/app/answers/detail/a_id/1787`,
  `https://aca.act.gov.au/licence-and-registration/check-a-licence`,
];
...
const BROAD_TERMS = ['building', 'constructions', 'pty', 'smith'];
const SEARCH_PARAMS = ['q', 'name', 'search', 'query', 'licenceName'];
...
function extractNames($) {
  ...
  for (const sel of candidates) {
    ...
    $(sel).each((_, el) => {
      ...
      if (t.length > 3 && t.length < 120 && !/^(name|licence|type|status|expiry|class|number|search|trade)/i.test(t)) {
        names.push(t);
      }
    });
...
async function fetchWithPuppeteer(query) {
  ...
  try {
    ...
    // Try filling a visible search input
    const inputSel = await page.$('input[type="search"], input[placeholder*="name" i], input[placeholder*="licence" i]');
    if (inputSel) {
...
(async () => {
  header('ACT Access Canberra — Licence Register Probe');
  let passed = 0;
  let failed = 0;

  step('Step 1: Probing ACT licence search...');
  let rawHtml = null;
  let usedTerm = null;
  ...
  const urlsToTry = [LICENCE_URL, ...ALT_URLS];
  ...
  for (const url of urlsToTry) {
    for (const term of (suppliedName ? [suppliedName] : BROAD_TERMS)) {
      for (const param of SEARCH_PARAMS) {
        try {
          const { data } = await axios.get(`${url}?${param}=${encodeURIComponent(term)}`, {
            headers: HEADERS, timeout: 20000,
          });
          if (data && data.length > 500) {
            rawHtml = data; usedTerm = term; usedParam = param; usedUrl = url; break outer;
          }
        } catch { /* try next */ }
      }
    }
  ...
  if (!rawHtml) {
    try {
      ...
      fail('Step 1', `All access methods failed.\nURL: ${LICENCE_URL}\nLast error: ${e.message}\n` +
        'ACT Access Canberra may have moved their licence check to a new portal.\n' +
        'Try visiting https://www.accesscanberra.act.gov.au manually.');
      summary(0, 1);
      process.exit(1);
    }
  }
  ...
  if (names.length === 0) {
    fail('Step 2',
      ...
      'ACT Access Canberra may redirect to a portal (JADU/ServiceNow) that requires\n' +
      'browser-based interaction or a different authentication mechanism.\n' +
  ...
  if (!found) {
    ...
    passed++;
    step(`SCRAPER NOTE: ACT licence check parseable via ${usedMethod}. Selector: "${sel}"`);
