...
function buildSearchUrl(entityName: string, entityAbn: string | null): string {
...
export function ReportCard({
  ...
  function handleRecheck() {
    if (freeChecks > 0) {
      // Navigate to search — the EmailGate will surface the re-check credit, save route consumes it
      router.push(buildSearchUrl(search.entityName, search.entityAbn));
    } else {
      setShowRecheckModal(true);
    }
  }

  function handleRecheckPaymentSuccess() {
    setShowRecheckModal(false);
    // Balance now credited; proceed to search flow
    router.push(buildSearchUrl(search.entityName, search.entityAbn));
  }
    ...
    <>
      <div className="bg-surface border border-border-light rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={handleRecheck}
            title={freeChecks > 0 ? `${freeChecks} re-check credit${freeChecks !== 1 ? 's' : ''} available` : 'Re-check for $3.00'}
          ...
          >
            Re-check{freeChecks > 0 ? ` (${freeChecks})` : ''}
          </button>
          <button
            onClick={handleShare}
      ...
      {showRecheckModal && (
        <PaymentModal
          ...
          metadata={{ entityName: search.entityName, ...(search.entityAbn ? { entityAbn: search.entityAbn } : {}) }}
          onSuccess={handleRecheckPaymentSuccess}
